...
<cfif IsDefined("SetDefault")>
	...
	<cfif Not IsDefined("NoBOBHist")>
		<cfquery name="GetOldFTP" datasource="#pds#">
	...
	<cfif Not IsDefined("NoBOBHist")>
		...
		<cfquery name="BOBHist" datasource="#pds#">
			...
			'#StaffMemberName.FirstName# #StaffMemberName.LastName# changed the default FTP from #GetOldFTP.FTPDescription# to #GetNewFTP.FTPDescription#.')
		</cfquery>
	</cfif>
</cfif>
	...
	<cftransaction>
		...
		<cfif Not IsDefined("NoBOBHist")>
			...
			<cfquery name="BOBHist" datasource="#pds#">
				...
				VALUES 
				(Null,0,#MyAdminID#, #Now()#,'System',
				'#StaffMemberName.FirstName# #StaffMemberName.LastName# deleted #GetFTP.FTPDescription# from the custom FTP setup.')
			</cfquery>
		</cfif>
	...
	<cftransaction>
		...
		<cfif Not IsDefined("NoBOBHist")>
			<cfquery name="BOBHist" datasource="#pds#">
				...
				VALUES 
				(Null,0,#MyAdminID#, #Now()#,'System',
				'#StaffMemberName.FirstName# #StaffMemberName.LastName# added #FTPDescription# to the custom FTP setup.')
			</cfquery>
		</cfif>
	</cftransaction>
	<cfquery name="CheckForFirst" datasource="#pds#">
		SELECT CustomFTPID 
		FROM CustomFTPSetup 
	</cfquery>
	<cfif CheckForFirst.RecordCount GT 0>
		<cfquery name="InsValues" datasource="#pds#">
			INSERT INTO CustomFTPSetup 
			(FTPDescription, BOBName, ActiveYN, CFVarYN, DataType, CFTPID, SortOrder, ScriptCode) 
			SELECT FTPDescription, BOBName, ActiveYN, CFVarYN, DataType, #CFTPID#, SortOrder, ScriptCode 
			FROM CustomFTPSetup 
			WHERE CFTPID = 
				(SELECT CFTPID 
				 FROM CustomFTP 
