...
<!---	4.0.2 01/26/01 Fixed an error when deleting the only user in the finish list caused the Wizard to not load the defaults.
		4.0.1 01/23/01 Fixed an error that caused a login loop when the start page was selected as the Account Wizard.
		4.0.0 08/14/99
			...
			<cfoutput query="CheckFirst">
				<tr>
					...
					<form method="post" action="account.cfm" onsubmit="return confirm('Click Ok to confirm deleting this Account Wizard Session.')">
						...
						<input type="Hidden" name="DelAccountID" value="#AccountID#">
