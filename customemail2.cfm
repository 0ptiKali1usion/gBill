...
<cfif (IsDefined("MvRt")) AND (IsDefined("TheHaveNots"))>
	...
	<cfif Not IsDefined("NoBOBHist")>
		...
		<cfquery name="MailName" datasource="#pds#">
		...
		<cfquery name="BOBHist" datasource="#pds#">
			...
			'#StaffMemberName.FirstName# #StaffMemberName.LastName# added the following domains to #MailName.EMailDescription#: #ValueList(TheNames.DomainName)#.')
		</cfquery>
	</cfif>
</cfif>
<cfif (IsDefined("MvLt")) AND (IsDefined("TheHaves"))>
	...
	<cfif Not IsDefined("NoBOBHist")>
		...
		<cfquery name="BOBHist" datasource="#pds#">
			...
			VALUES 
			(Null,0,#MyAdminID#, #Now()#,'System',
			'#StaffMemberName.FirstName# #StaffMemberName.LastName# removed the following domains from #MailName.EMailDescription#: #ValueList(TheNames.DomainName)#.')
		</cfquery>
	</cfif>
</cfif>
<cfif IsDefined("UpdSelected.x")>
	...
	<cfloop query="GetFields">
		<cfif IsDefined("SortOrder#CustomEMailID#")>
			<cfset TheSort = Evaluate("SortOrder#CustomEMailID#")>
		<cfelse>
			<cfset TheSort = GetFields.RecordCount>
		</cfif>
		<cfquery name="UpdData" datasource="#pds#">
			UPDATE CustomEMailSetup SET 
			SortOrder = #TheSort# 
			WHERE CustomEMailID = #CustomEMailID# 
		</cfquery>
	</cfloop>
	<!--- BOB History --->
	<cfif Not IsDefined("NoBOBHist")>
		<cfquery name="GetNewMail" datasource="#pds#">
			SELECT EMailDescription 
			FROM CustomEMail 
			WHERE CemailID = #CemailID# 
		</cfquery>
		<cfquery name="BOBHist" datasource="#pds#">
			...
			VALUES 
			(Null,0,#MyAdminID#, #Now()#,'System',
			'#StaffMemberName.FirstName# #StaffMemberName.LastName# edited #GetNewMail.EMailDescription#.')
		</cfquery>
	</cfif>
</cfif>
...
<cfif Tab Is 1>
	...
	<cfquery name="GetFields" datasource="#pds#">
		...
		AND BOBName <> 'DomainName' 
		AND BOBName <> 'UserName' 
		ORDER BY ActiveYN desc, SortOrder, EMailDescription 
	</cfquery>
	<cfquery name="ActiveNum" datasource="#pds#">
		SELECT * 
		FROM CustomEMailSetup 
		WHERE CemailID = #CemailID# 
		AND BOBName <> 'EPass' 
			...
			<cfloop query="GetFields">
				<cfoutput>
					<tr bgcolor="#tbclr#">
						...
						<cfif ActiveYN Is 1>
							<td><select name="SortOrder#CustomEMailID#">
								<cfloop index="B5" from="1" to="#ActiveNum.RecordCount#">
