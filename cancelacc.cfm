...
<cfif IsDefined("Report.x")>
	...
	<cfquery name="GetCanReason" datasource="#pds#">
		SELECT CxReason
		FROM LU_CxReason L, Accounts A
		WHERE  A.CancelReason = L.CxReasonID
		GROUP BY A.CancelReason, L.CxReasonID, L.CxReason
	</cfquery>
	<cfset CancelReason = GetCanReason.CxReason>
	<cfquery name="getaccounts" datasource="#pds#">
		...
		'#CancelReason#', 6, #MyAdminID#, 'Cancelled between #LSDateFormat(Date1, '#DateMask1#')# and #LSDateFormat(Date2, '#DateMask1#')#', #Now()#, 
