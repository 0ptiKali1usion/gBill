...
<cfif IsDefined("AcknowledgeRead")>
	...
	<cfif Not IsDefined("NoBOBHist")>
		<cfquery name="GetMessageInfo" datasource="#pds#">
			SELECT Message 
			FROM StaffMessages 
		...
		<cfquery name="BOBHist" datasource="#pds#">
			...
			VALUES 
			(Null,0,#MyAdminID#, #Now()#,'Staff Messages',
			'#StaffMemberName.FirstName# #StaffMemberName.LastName# acknowledged reading the staff message: #GetMessageInfo.Message#.')
		</cfquery>
	</cfif>	
</cfif>
			...
			<td>
				<table border="#tblwidth#" width="100%">
					...
					<cfoutput query="menuheaders">
						...
						<th bgcolor=<cfif level1 Is LevelID>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><a href="admin.cfm?level1=#levelid#">#levelname#</a></th>
						<cfif counter is 3></tr><cfset counter = 0></cfif>
						<cfset counter = counter + 1>
					</cfoutput>
					<cfoutput>
						<cfif counter is 3><td bgcolor="#ttNTab#"> </td></tr><cfset counter = 0></cfif>
						<cfif counter is 2><td bgcolor="#ttNTab#"> </td><td bgcolor="#ttNTab#"> </td></tr><cfset counter = 0></cfif>
					</cfoutput>
				</table>
			</td>
		</tr>
