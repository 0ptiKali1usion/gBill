...
<cfif IsDefined("AddMenu.x")>
	<cftransaction>
		...
		<cfquery name="SetUsers" datasource="#pds#">
			SELECT AdminID 
			FROM Admin 
			WHERE SUserYN = 1 
		</cfquery>
		<cfloop query="SetUsers">
			<cfquery name="GiveMenu" datasource="#pds#">
				INSERT INTO Connect 
				(AdminID, MenuID) 
				VALUES 
				(#AdminID#, #MenuID#) 
				...
				<table border="1">
					<tr>
						<form method="post" action="editmenu.cfm">
							...
							<td bgcolor=<cfif tab Is 2>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><input type="radio" <cfif tab Is 2>checked</cfif> name="tab" value="2" onclick="submit()" id="tab2"><label for="tab2">General</label></td>
