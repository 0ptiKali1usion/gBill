...
<!---	4.0.2 01/04/01 Fixed the non prorating for multi month plans
		4.0.1 01/03/01 Fixed the prorating for multi month plans 
		4.0.0 08/14/99 --->
<!--- account2.cfm --->
...
<cfloop query="PlanInfo">
	...
	<cfif ProratePYN Is 1>
		...
		<cfif TodayDate Is NextDue>
			...
			<cfset NumDays = 100>
		<cfelseif TodayDate LT NextDue>
			<cfset NumDays = DateDiff("d",TodayDate,NextDue)>
			<cfif NumDays LT ProrateCutDays>
				<!--- Add the Extra --->
				<cfset Date1 = DateAdd("m",-1,NextDue)>
				<cfset Date2 = DateAdd("m",RecurringCycle,Date1)>
				<cfset NumDaysDiff = DateDiff("d",Date1,Date2)>
				<cfset NumDaysPer = DateDiff("d",TodayDate,NextDue)>
				<cfset CostPerDayRA = RecurringAmount/NumDaysDiff>
				<cfset CostPerDayRD = RecurDiscount/NumDaysDiff>
				<cfset CurRA = NumDaysPer * CostPerDayRA>
				<cfset CurRD = NumDaysPer * CostPerDayRD>
			<cfelse>
				<!--- Subtract the difference --->
				<cfset Date1 = DateAdd("m",-1,NextDue)>
				<cfset Date2 = DateAdd("m",RecurringCycle,Date1)>
				<cfset NumDaysDiff = DateDiff("d",Date1,Date2)>
				<cfset NumDaysPer = DateDiff("d",Date1,TodayDate)>
				<cfset CostPerDayRA = RecurringAmount/NumDaysDiff>
				<cfset CostPerDayRD = RecurDiscount/NumDaysDiff>
				<cfset CurRA = RecurringAmount - (NumDaysPer * CostPerDayRA)>
				<cfset CurRD = RecurDiscount - (NumDaysPer * CostPerDayRD)>
				<cfset NextDue = Date2>
			</cfif>
		</cfif>
	<cfelse>
	...
	<cfif CurRD GT 0>
		...
		<tr bgcolor="#tbclr#">
			...
			<cfif ProRateYN is 1>
				<cfset DispStr = DateAdd("s",-1,NextDue)>
				<cfset MemoStr = RDMemo & " Prorated for #LSDateFormat(TodayDate, '#DateMask1#')# to #LSDateFormat(DispStr, '#DateMask1#')#.">
			<cfelse>
