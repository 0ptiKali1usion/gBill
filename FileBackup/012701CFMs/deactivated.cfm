...
<cfset ReportSecure = "deactivated.cfm">
<cfset ReportID = 14>
<cfset LetterID = 14>
<cfset ShowFilters = "1">
...
<cfset ReportTitle = "Deactivated Accounts">
<cfset HowWide = "2">
...
<cfif IsDefined("Report.x")>
	...
	<cfquery name="allcredits" datasource="#PDS#">
		...
		P.DeactReason, #ReportID#, #MyAdminID#, 'Temporarily Deactivated - #LSDateFormat(Date1, '#DateMask1#')# to #LSDateFormat(Date2, '#DateMask1#')#', #Now()# 
		FROM Accounts A, AccntPlans P 
		WHERE A.AccountID = P.AccountID 
		AND A.DeactivatedYN = 1 
		AND A.CancelYN = 0 
		AND A.DeactDate <= #CreateODBCDateTime(Date2)# 
		AND A.DeactDate >= #CreateODBCDateTime(Date1)# 
