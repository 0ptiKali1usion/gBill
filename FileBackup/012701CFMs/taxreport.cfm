...
<cfif IsDefined("CreateReport.x")>
	...
	<cfelseif ReportType Is "Sales">
		<cfquery name="TaxInfo" datasource="#pds#">
			...
			<cfif DomainID Is Not "0">
				AND 
				(T.AuthDomainID In (0,#DomainID#) 
				 OR T.EMailDomainID In (0,#DomainID#) 
				 OR T.FTPDomainID In (0,#DomainID#) 
				)
			<cfelse>
				AND 
				(T.AuthDomainID In 
...
<cfif CheckFirst.Recordcount Is 0>
	...
	<form name="getdate" method="post" action="taxreport.cfm?RequestTimeout=300" onsubmit="return checkdates();MsgWindow()">
		...
		<tr bgcolor="#tbclr#">
			<td>&nbsp;</td>
			<td bgcolor="#tdclr#" align="right"><select name="ReportType">
				<option value="Domains">Domains
				<option value="Plans">Plans
				<option value="POPs">POPs
				<option value="Sales">Salesperson
			</select></td>
			<td colspan="2">Type of Report</td>
		</tr>
		<tr>
