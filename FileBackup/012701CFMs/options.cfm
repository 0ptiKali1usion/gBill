...
<cfif tab Is 1>
	...
	<cfquery name="BOBHistory" datasource="#pds#">
		SELECT AutoLoadYN 
		FROM Setup 
		WHERE VarName = 'NoBOBHist' 
	</cfquery>
	<cfquery name="BOBPHistory" datasource="#pds#">
		SELECT AutoLoadYN 
		FROM Setup 
		WHERE VarName = 'NoBOBPHist' 
	</cfquery>
...
<cfelseif tab Is 4>
	...
	<cfquery name="AllCCTypes" datasource="#pds#">
		...
		Order By ActiveYN desc, SortOrder, CardType
	</cfquery>
...
<cfelseif tab Is 5>
	<cfset HowWide = 2>
	<cfquery name="GetCMSetup" datasource="#pds#">
		SELECT * 
		FROM Setup 
		WHERE VarName = 'AllowedNames' 
	</cfquery>
<cfelseif tab Is 24>
	<cfset HowWide = 3>
<cfelseif tab Is 25>
		...
		<tr>
			<th colspan="#HowWide#">
				<table border="1">
					<tr>
						<form method="post" action="options.cfm">
							<th bgcolor=<cfif Tab Is 1>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><input type="radio" <cfif tab Is 1>checked</cfif> name="tab" value="1" onClick="submit()" id="tab1"><label for="tab1">gBill System</label></th>
							<th bgcolor=<cfif Tab Is 5>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><input type="radio" <cfif tab Is 5>checked</cfif> name="tab" value="5" onClick="submit()" id="tab5"><label for="tab5">Setup</label></th>
							<th bgcolor=<cfif Tab Is 2>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><input type="radio" <cfif tab Is 2>checked</cfif> name="tab" value="2" onClick="submit()" id="tab2"><label for="tab2">Company</label></th>
							<th bgcolor=<cfif Tab Is 4>"#ttSTab#"<cfelse>"#ttNTab#"</cfif> ><input type="radio" <cfif tab Is 4>checked</cfif> name="tab" value="4" onClick="submit()" id="tab4"><label for="tab4">Credit Cards</label></th>
...
<cfelseif tab Is 5>
	<cfinclude template="opttab5.cfm">
<cfelseif tab Is 24>
	<form method="post" action="options.cfm">
		<cfoutput>
			<input type="hidden" name="tab" value="4">
...
<cfelseif tab Is 25>
	<form method="post" action="options.cfm">
		<cfoutput>
			<input type="Hidden" name="tab" value="5">
			<tr>
				<td align="center" bgcolor="#tbclr#">IP Address</td>
				<td bgcolor="#tdclr#"><input type="Text" name="NewValue1"></td>
				<input type="Hidden" name="NewValue1_Required" value="Please enter the IP address that the Customer Support module is installed at.">
			</tr>
			<tr>
				<th colspan="2"><input type="image" name="EnterIP" src="images/enter.gif" border="0"></th>
			</tr>
		</cfoutput>
	</form>
</cfif>
