...
<cfloop query="getfileinfo">
	...
	<cfif ftpyn is 1>
		<cfset thefilename = "#ciid#-#LSDateFormat(now(), 'mmm-dd-yy')#-#TimeFormat(now(), 'hh-mm-ss')#.rad">
		...
		<cfif thedir is not "">
			<cfftp action="changedir" connection="thisfile" directory="#thedir#">
		</cfif>
		<cfftp action="RENAME" existing="#filename1#" new="#thefilename#" connection="thisfile">
		<cfftp action="getfile" connection="thisfile" remotefile="#thefilename#" localfile="#pathimport##thefilename#">
	...
	<cfelse>
		...
		<cfif FileExists("#server1##path1##filename1#")>
			...
			<cffile action="RENAME"
			 source="#server1##path1##thefilename#"
			 destination="#server1##path1##filename1#">
