...
<cfif IsDefined("ActiveStatus")>
		<cfquery name="SearchResults" datasource="#pds#">
			...
			(ReportID,AdminID,AccountID,FirstName,Lastname,City,Address,CurTime, AccntPlanID,
			 CurPercent,Phone,Company,PhoneWk, Login, EMail, CreateDate)
			SELECT 4,#MyAdminID#,A.AccountID,A.FirstName,A.Lastname,A.City,
			A.Address1, A.DeactivatedYN, A.CancelYN, #savesearch#, 
...
<cfset HowWide = 9>
<cfset CompanyList = ValueList(TheResults.Company)>
<cfset CompanyList = ReplaceList(CompanyList," ","")>
<cfset CompCheck = ListLen(CompanyList)>
<cfif CompCheck Is 0>
	<cfset HowWide = HowWide - 1>
	<cfset NoCompany = 1>
</cfif>
<cfset MailList = ValueList(TheResults.EMail)>
<cfset MailList = ReplaceList(MailList," ","")>
<cfset MailCheck = ListLen(MailList)>
<cfif MailCheck Is 0>
	<cfset HowWide = HowWide - 1>
	<cfset NoEMail = 1>
</cfif>
...
<html>
<head>
<cfoutput><title>#TheResults.RecordCount# Results</title></cfoutput>
...
<table border="#tblwidth#">
	<tr>
		<th colspan="#HowWide#" bgcolor="#ttclr#"><font size="#ttsize#" <cfif ttface Is Not "NA">face="#ttface#"</cfif> color="#ttfont#">Search Results</font></th>
	</tr>
</cfoutput>
...
<cfoutput>
	...
	<cfelse>
		<tr bgcolor="#tbclr#">
			<td colspan="#HowWide#">No results were found for your search.<br>
			To change the current search criteria click 'Change Criteria'.</td>
		</tr>
		<tr>
			<form method="post" action="lookup1.cfm">
				<th colspan="#HowWide#"><input type="image" name="Change" src="images/changecriteria.gif" border="0"></th>
			</form>
		</tr>
	</cfif>
</cfoutput>
<cfoutput query="TheResults" startrow="#Srow#" maxrows="#MaxRows#">
	<tr bgcolor="#tbclr#" valign="top">
		<form method="post" <cfif getopts.OpenNew Is 1>target="_#FirstName##LastName#"</cfif> action="custinf1.cfm">
			<th bgcolor="#tdclr#"><input type="Radio" name="AccountID" value="#AccountID#" onclick="submit()"></th>
		</form>
		<td nowrap><cfif AccntPlanID Is 1>* <cfelseif CurTime Is 1>+ </cfif>#LastName#, #FirstName#</td>
		<cfif NoCompany Is 0>
			<td><cfif Trim(Company) Is "">&nbsp;<cfelse>#Company#</cfif></td>
		</cfif>
		<cfif FindNoCase(" ",Login)>
			<cfset Pos1 = FindNoCase(" ",Login)>
			<td>#Left(Login,Pos1)#</td>
		<cfelse>
		...
		<form method="post" action="lookup.cfm">
			<th bgcolor="#tdclr#"><input type="Radio" name="RemoveOne" value="#GrpListID#" onclick="submit()"></th>
		</form>
	</tr>
	<cfif (AccntPlanID Is 1) AND (CancelView Is 0)>
		<cfset CancelView = 1>
	</cfif>
	<cfif (CurTime Is 1) AND (DeactView Is 0)>
		<cfset DeactView = 1>
	</cfif>
</cfoutput>
	<cfif (TheResults.RecordCount GT 0) AND (CancelView Is 1 OR DeactView Is 1)>
		<tr>
			<cfoutput>
				<td bgcolor="#tdclr#">&nbsp;</td>
				<td bgcolor="#tbclr#" colspan="#HowWide2#"><cfif DeactView Is 1>+ = deactivated</cfif><cfif DeactView Is 1 AND CancelView Is 1><br></cfif><cfif CancelView Is 1>* = cancelled</cfif></td>
			</cfoutput>
		</tr>
	</cfif>
	<cfif TheResults.RecordCount GT Mrow>
