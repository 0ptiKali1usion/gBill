...
<cfif IsDefined("DelAuth.x")>
	...
	<cfquery name="GetMain" datasource="#pds#">
		SELECT EMail, AccountID, FName, LName 
		FROM AccountsEMail 
		WHERE EMailID = #EMailID# 
	</cfquery>
	<cfif (Not IsDefined("NoBOBHist")) AND (GetAlias.RecordCount GT 0)>
		<cfquery name="BOBHist" datasource="#pds#">
			INSERT INTO BOBHist
			(ActionEmail, AccountID, AdminID,  ActionDate, Action, ActionDesc) 
			VALUES 
			(Null,#GetMain.AccountID#,#MyAdminID#, #Now()#,'Edited Customer Info',
			 '#StaffMemberName.FirstName# #StaffMemberName.LastName# deleted the following email alias: #ValueList(GetAlias.EMail)# for #GetMain.FName# #GetMain.LName#.')
		</cfquery>
	</cfif>
	<cfif Not IsDefined("NoBOBHist")>
		<cfquery name="BOBHist" datasource="#pds#">
			...
			VALUES 
			(Null,#GetMain.AccountID#,#MyAdminID#, #Now()#,'Edited Customer Info',
			 '#StaffMemberName.FirstName# #StaffMemberName.LastName# deleted the email address: #GetMain.EMail# for #GetMain.FName# #GetMain.LName#.')
		</cfquery>
	</cfif>
