...
<!--- Version 4.0.0 --->
<!---	4.0.1 01/17/01 Changed the length of the test messages.
		4.0.0 03/03/00 --->
<!--- charge.cfm --->
<!--- Syntax:  <CF_Charge
	...
	<cfif CCErrorLevel is 0>
		...
		<cfif GoBack is 0>
			...
			<cfelseif CCPack is "cyber">
				<!--- CyberCash --->		
				<cfset ORDER = "0#LSDateFormat(NOW(),'YYMMDD')##TimeFORMAT(NOW(),'HHmmss')#">
		 	...
		 	<cfelseif CCPack is "formbased">
				...
				<cfquery name="GetNextNumber" datasource="#pds#">
					SELECT FieldValue 
					FROM CustomCCOutput 
					WHERE FieldName1 = 'SeqNumber' 
					AND UseTab = 6 
				</cfquery>
				<cfif GetNextNumber.RecordCount GT 0>
					<cfset UniqueNumber = GetNextNumber.FieldValue + 1>
				<cfelse>
					<cfquery name="InsSeqNumber" datasource="#pds#">
						INSERT INTO CustomCCOutput 
						(FieldName1, Description1, UseYN, SortOrder, CFVarYN, UseTab, FieldValue) 
						VALUES 
						('SeqNumber','Unique Number',1,Null,1,6,'1')
					</cfquery>
					<cfset UniqueNumber = 1>
				</cfif>
				<cfquery name="UpdCurrent" datasource="#pds#">
					UPDATE CustomCCOutput SET 
					FieldValue = '#UniqueNumber#' 
					WHERE FieldName1 = 'SeqNumber' 
					AND UseTab = 6 
				</cfquery>
				<cfset BOBHistMess = "Credit Card Transaction was submitted to: #URL#">
				<cfhttp url="#URL#" method="POST">
					<cfloop query="GetFormFields">
						<cfset WorkVar = FieldValue>
						<cfset SendValue = ReplaceList("#WorkVar#","%CC00,%CC01,%CC02,%CC03,%CC04,%CC05,%CC06,%CC07,%CC08,%CC09,%CC10,%CC11,%CC12","#TheID#,#TheFName#,#TheLName#,#TheAddress#,#ThePhone#,#Attributes.AVSZip#,#TheEM#,#Attributes.Amount#,#Attributes.Card#,#TheExpMonth#,#TheExpYear#,#TheCardType#,#UniqueNumber#")>
						<cfset BOBHistMess = BOBHistMess & "  #FieldName1# = #SendValue#.">
				...
				<cfquery name="GetResponseItems" datasource="#pds#">
					...
					FROM CustomCCOutput 
					WHERE UseTab = 8 
					AND FieldName1 In ('ApproveDelimiter','ResponseField','CodeField','MessageField','DecMessageField', 
					'CodeFieldName','DecMessageFieldName','MessageFieldName','ResponseFieldName') 
				</cfquery>
				<cfloop query="GetResponseItems">
					<cfset "Var#FieldName1#" = FieldValue>
				</cfloop>
				<cfif VarApproveDelimiter Is "">
					<cfset Pos1 = FindNoCase(VarResponseFieldName,Result1)>
					<cfset Pos1a = FindNoCase("&",Result1,Pos1)>
					<cfset Len1 = Pos1a - Pos1>
					<cfset TheResponse = Mid(Result1,Pos1,Len1)>
					<cfset ApproveYN = FindNoCase("APPRVD",TheResponse)>
					<cfif ApproveYN GT 0>
						<cfset TheRes = "APPRVD">
					<cfelse>
						<cfset TheRes = "Declined">
					</cfif>
					<cfif TheRes Is "Declined">
						<cfset LocMes = Replace(TheResponse,"terminalResponse=","")>
						<cfset LocMes = URLDecode(LocMes)>
						<cfset LocMes = Trim(LocMes)>
						<cfset TheCde = "Declined">
						<cfset TheMes = LocMes>
						<cfset TheDec = LocMes>
					<cfelse>
						<cfset LocMes = Replace(TheResponse,"terminalResponse=APPRVD%20%20%23%20-","")>
						<cfset TheCde = LocMes>
						<cfset TheMes = "Approved">
						<cfset TheDec = "NA">
					</cfif>
				<cfelse>
					...
					<cfset TheDec = ListGetAt("#Result1#","#VarDecMessageField#","#VarApproveDelimiter#")>
				</cfif>
				<cfif TheRes Is TheApprovalCodeIs>
					<cfset CCResponse = "OK">
					<cfif Trim(TheCde) Is Not "">
						<cfset TheCde = URLDecode(TheCde)>
						<cfset TheCde = ReplaceList(TheCde,"terminalResponse=","")>
						<cfset TheCde = Trim(TheCde)>
					</cfif>
				...
				<cfelse>
					<cfset CCResponse = "No">
					<cfset CCCodeAnswer = "Problem"> 
					<cfif Trim(TheDec) Is Not "">
						<cfif FindNoCase("%",TheDec)>
							<cfset TheDec = URLDecode(TheDec)>
							<cfset TheDec = ReplaceList(TheDec,"terminalResponse=","")>
							<cfset TheDec = Trim(TheDec)>
						</cfif>
						<cfset CCMessage = TheDec>
					<cfelse>
						<cfset CCMessage = "Problem"> 
					</cfif>
					<cfset CCErrorLevel = "1">
				...
				<cfif IsDefined("MyAdminID")>
					<cfif Not IsDefined("NoBOBHist")>
						...
						<cfset GetInfoAccountID = 0>
						<cfif IsDefined("Attributes.AccountID")>
							<cfquery name="GetWho" datasource="#pds#">
								SELECT FirstName, LastName 
								FROM Accounts 
								WHERE AccountID = #Attributes.AccountID#
							</cfquery>
							<cfif GetWho.RecordCount Is 1>
								<cfset BOBHistMess = "#StaffMemberName.FirstName# #StaffMemberName.LastName# submited a credit card transaction for #GetWho.FirstName# #GetWho.LastName#.  The response was #CCResponse#.  " & BOBHistMess>
								<cfset GetInfoAccountID = Attributes.AccountID>
							</cfif>
						<cfelse>
							<cfset BOBHistMess = "#StaffMemberName.FirstName# #StaffMemberName.LastName# submited a credit card transaction.  The response was #CCResponse#.  " & BOBHistMess>							
						</cfif>
						<cfquery name="BOBHist" datasource="#pds#">
							INSERT INTO BOBHist 
							(ActionEmail, AccountID, AdminID,  ActionDate, Action, ActionDesc) 
							VALUES 
							(Null,#GetInfoAccountID#,#MyAdminID#, #Now()#,'Financial',
							 '#BOBHistMess#  The response was: #Result1#')
						</cfquery>
					</cfif>
				</cfif>
			<cfelse>
...
<cfelse>
	...
	<cfif TestVar2 Is 1>
		<cfset CCResponse = "No">
		<cfset CCCodeAnswer = "Problem">
		<cfset CCMessage = "Test: Declined.">
		<cfset CCErrorLevel = "1">
	<cfelse>
		<cfset CCResponse = "Ok">
		<cfset CCCodeAnswer = "A1234567">
		<cfset CCMessage = "Test: Approved.">
		<cfset CCErrorLevel = "0">
	</cfif>
</cfif>
