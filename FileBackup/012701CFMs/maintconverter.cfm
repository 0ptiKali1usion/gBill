...
<!--- Version 4.0.0 --->
<!--- 4.0.1 01/16/01 Added the capture IP feature for setting up the support module.
		4.0.0 08/22/00 --->
<!--- maintconverter.cfm --->
<!--- 4 Types 
Query Inputs
...
--->
<cfquery name="CheckDebug" datasource="#pds#">
	SELECT Value1, Description 
	FROM Setup 
	WHERE VarName = 'GetTheIP' 
</cfquery>
<cfif CheckDebug.RecordCount GT 0>
	<cfset GetTheIP = CheckDebug.Value1>
	<cfif GetTheIP Is "1">
		<cfquery name="EnterIP" datasource="#pds#">
			UPDATE Accounts SET Notes = '#CGI.Remote_addr#' 
			WHERE Login = 
			<cfif CheckDebug.Description Is Not "">
				'#CheckDebug.Description#'
			<cfelse>
				'gbill'
			</cfif>
		</cfquery>
...
<cfparam name="NoAllow" default="0">
<cfquery name="GetAllowed" datasource="#pds#">
	SELECT Value1 
	FROM Setup 
	WHERE VarName = 'AllowedNames' 
</cfquery>
<cfset NameToCheck = CGI.Remote_addr>
<cfif NameToCheck Is "">
	<cfset NoAllow = 1>
</cfif>
<cfset ServerList = ValueList(GetAllowed.Value1)>
<cfif ListFind(ServerList,NameToCheck) Is 0>
	<cfset NoAllow = 1>
</cfif>
...
<cfif NoAllow Is 1>
	...
	<center>
		<table border="3">
			<tr>
				<th colspan="2" bgcolor="royal blue"><font color="yellow" size="5">Currently Unavailable</font></th>
			</tr>
			<tr>
				<td colspan="2" bgcolor="ffffff">Site currently unavailable.  Error code 112294.</td>
			</tr>
		</table>
	</center>
	</body>
...
<cfif ResultType Is "Query">
	<cfquery name="CheckDebugModule" datasource="#pds#">
		SELECT Value1 
		FROM Setup 
		WHERE VarName = 'TurnOnDebug'
	</cfquery>
	<cfif (CheckDebugModule.RecordCount Is 1) AND (CheckDebugModule.Value1 Is 1)>
