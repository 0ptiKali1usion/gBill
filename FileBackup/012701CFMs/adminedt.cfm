...
--->
<!--- 4.0.1 Fixed an error with the Clean up for personal sorting.
		4.0.0 --->
<!-- adminedt.cfm -->
...
<cfif (IsDefined("DupPermissions.x")) AND (IsDefined("CopyFrom")) AND (IsDefined("CopyTo"))>
	...
	<cfif CopyTo Is Not "">
		...
		<cfquery name="UpdTo" datasource="#pds#">
			...
			chplan = #achplan#, waivea = #awaivea#, deactc = #adeactc#, cancelc = #acancelc#, 
			cancela = #acancela#, deltrans = #adeltrans#, schevent = #aschevent#, 
		...
		<cfif Not IsDefined("NoBOBHist")>
			...
			<cfquery name="BOBHist" datasource="#pds#">
				...
				'#StaffMemberName.FirstName# #StaffMemberName.LastName# copied the permissions for #GetWhoName.FirstName# #GetWhoName.LastName# to these staff members: #ValueList(TheNames.FullName)#.')
			</cfquery>
		</cfif>
...
<cfquery name="LocaleInfo" datasource="#pds#">
	SELECT Value1, VarName 
	FROM Setup 
	WHERE VarName In ('DateMask1','Locale') 
</cfquery>
<cfloop query="LocaleInfo">
...
<form method=post action="adminedt.cfm?RequestTimeout=500" name="EditInfo">	
	<cfoutput query="alladmin">
		<tr valign="top">
			...
			<cfif GetOpts.SUserYN Is "1">
				<td bgcolor="#tbclr#">#LSDateFormat(lastsess, '#DateMask1#')# #TimeFormat(lastsess, 'hh:mm tt')#</td>
			</cfif>
