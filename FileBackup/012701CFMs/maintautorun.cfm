...
Cancel
	...
	AuthID - The actual auth account to be cancelled.  If 0 then all for the AccntPlanID are cancelled.
	Memo1 - The reason.
		...
		<cfelseif DoAction Is "Rollback">
			...
			<cfelseif PlanID Is DelAccount>
				<cfquery name="SchCancel" datasource="#pds#">
					INSERT INTO AutoRun 
					(PlanID, AccountID, AccntPlanID, AuthID, Memo1, Memo2, ScheduledBy, DoAction, WhenRun, Value1)
					VALUES 
					(#PlanID#, #AccountID#, #AccntPlanID#, <cfif AuthID Is "">Null<cfelse>#AuthID#</cfif>, 
					 <cfif Trim(Memo1) Is "">Null<cfelse>'#Memo1#'</cfif>, 
					 <cfif Trim(Memo2) Is "">Null<cfelse>'#Memo2#'</cfif>, 'gBill Auto Hour Check', 'Cancel',#Now()#, 0)
				</cfquery>
				<cfquery name="BOBHist" datasource="#pds#">
					INSERT INTO BOBHist 
					(AccountID, AdminID, ActionDate, Action, ActionDesc)
