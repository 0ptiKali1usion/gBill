...
<cfif tab Is 1>
	<cfquery name="Titles" datasource="#pds#">
		...
		<cfelse>
			FROM Levels L Left Join MenuItems M 
			ON L.LevelID = M.Menu 
		</cfif>
	...
	<tr>
		<th colspan="3">
			<table border="1">
				<tr>
					<form method="post" action="editlev.cfm">
						<th bgcolor=<cfif tab Is 1>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab Is 1>checked</cfif> name="tab" value="1" onclick="submit()" id="tab1"><label for="tab1">Headers</label></th>
						<th bgcolor=<cfif tab Is 2>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab Is 2>checked</cfif> name="tab" value="2" onclick="submit()" id="tab2"><label for="tab2">Sort Order</label></th>
					</form>
				</tr>
			</table>
		</th>
