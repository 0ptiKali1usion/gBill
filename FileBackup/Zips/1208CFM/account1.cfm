...
<cfif IsDefined("AddAccount.x")>
	...
	<cftransaction>
		<cfquery name="TempInfo" datasource="#pds#">
			...
			(#ValueList(GetFields.BOBFieldName)#,AdminID,TabCompleted,StartDate<cfif Tab Is 1>,WaiveA,CardHold</cfif>)
			VALUES 
			(<cfloop index="B5" list="#ValueList(GetFields.BOBFieldName)#">
				...
				<cfif Trim(FieldValue) Is "">NULL<cfelse><cfif DataType Is "Text">'#FieldValue#'<cfelseif DataType Is "Number">#FieldValue#<cfelseif DataType Is "Date">#CreateODBCDateTime(FieldValue)#</cfif></cfif>,
		...
		<th colspan="#HowWide#">
			<table border="1">
				<form method="post" action="account1.cfm">
					...
					<tr valign="top">
						<th bgcolor=<cfif Tab Is 1>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 1>checked</cfif> name="tab" value="1" onclick="submit()" id="Tab1"><label for="Tab1">Personal</label></th>
						<cfif CompletedTab GT 0>
							<th bgcolor=<cfif Tab Is 2>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 2>checked</cfif> name="tab" value="2" onclick="submit()" id="Tab2"><label for="Tab2">Support</label></th>
						<cfelse>
							<th bgcolor="#thclr#">Support</th>
						</cfif>
						<cfif CompletedTab GT 1>
							<th bgcolor=<cfif Tab Is 3>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 3>checked</cfif> name="tab" value="3" onclick="submit()" id="Tab3"><label for="Tab3">Service</label></th>
						<cfelse>
							<th bgcolor="#thclr#">Service</th>
						</cfif>
						<cfif CompletedTab GT 2>
							<th bgcolor=<cfif Tab Is 4>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 4>checked</cfif> name="tab" value="4" onclick="submit()" id="Tab4"><label for="Tab4">Integration</label></th>
						<cfelse>
							<th bgcolor="#thclr#">Integration</th>
						</cfif>
						<cfif CompletedTab GT 3>
							<th bgcolor=<cfif Tab Is 5>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 5>checked</cfif> name="tab" value="5" onclick="submit()" id="Tab5"><label for="Tab5">Financial</label></th>
						<cfelse>
							<th bgcolor="#thclr#">Financial</th>
						</cfif>
						<cfif CompletedTab GT 4>
							<th bgcolor=<cfif Tab Is 6>"#tbclr#"<cfelse>"#tdclr#"</cfif> ><input type="radio" <cfif tab is 6>checked</cfif> name="tab" value="6" onclick="submit()" id="Tab6"><label for="Tab6">Final Verification</label></th>
						</cfif>
					</tr>
				</form>
			</table>
