...
<!--- Version 4.0.0 --->
<!--- Account management. --->
<!---	4.0.1 02/20/01 Fixed error with double CFTransAction when calling the custom tag cfpayment.
		4.0.0 04/10/00 --->
<!--- accntmanage7.cfm --->
		...
		<cfif (CreditAmount GT 0) OR (DebitAmount GT 0)>
			<cfquery name="MakeTrans" datasource="#pds#">
				...
				(#PrimaryAccountID#, #CreateODBCDateTime(Now())#, #CreditAmount#, #DebitAmount#, '#DReason#', 0, '#StaffMemberName.FirstName# #StaffMemberName.LastName#', 0, 
				 ...
				 Null, 0, 0, 0, Null, Null, '#AccntPlanInfo.PayBy#', #PersInfo.SalesPersonID#, #AccntPlanID#, #DebitAmount#, #CreditAmount#, 0, 
			...
			<cfset SynchPayments = "1">
		</cfif>
	</cftransaction>
	<cfif IsDefined("SynchPayments")>
