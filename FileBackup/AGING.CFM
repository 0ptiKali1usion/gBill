...
<cfif IsDefined("CreateReport.x")>
	...
	<cfelse>
		...
		<cfelse>
			<cfquery name="StartReport" datasource="#pds#">
				INSERT INTO AgingTemp (AccountID, FirstName, LastName, Login, Phone, 
				AdminID, ReportDate, ReportID, ReportName)	
				SELECT a.AccountID, a.FirstName, a.LastName, a.Login, a.dayphone, 
				#MyAdminID#, #MyDate#, #ReportID#, '#ReportName#', 
				FROM accounts a, Transactions b 
				WHERE a.accountid = b.accountid 
			...
			<cfquery name="UpdEMail" datasource="#pds#">
				UPDATE AgingTemp, AccountsEMail 
				SET AgingTemp.EMailAddr = AccountsEMail.EMail 
				WHERE AgingTemp.AccountID = AccountsEMail.AccountID 
				AND AccountsEMail.PrEMail = 1
			</cfquery>
			<!--- Update CurChr, Past30, Past60, Past90, LastPayDt, LastPayAm, Total Pay, CurBal --->
			...
			<cfloop query="GetWho">
				<cfquery name="GetCurChr" datasource="#pds#">
					SELECT Sum(Debit) as CurChr 
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND datetime1 > #MyDate30# AND datetime1 <= #MyDate#
				</cfquery>
				<cfquery name="GetPast30" datasource="#pds#">
					SELECT Sum(Debit) as Past30 
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					 AND datetime1 > #MyDate60# AND datetime1 <= #MyDate30#
				</cfquery>
				<cfquery name="GetPast60" datasource="#pds#">
					SELECT Sum(Debit) as Past60 
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND datetime1 > #MyDate90# AND datetime1 <= #MyDate60#
				</cfquery>
				<cfquery name="GetPast90" datasource="#pds#">
					SELECT Sum(Debit) as Past90
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND datetime1 <= #MyDate90# 
				</cfquery>
				<cfquery name="GetLastPay" datasource="#pds#">
					SELECT max(datetime1) as MaxPayDate 
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND credit > 0 AND datetime1 <= #MyDate# 
				</cfquery>
				<cfquery name="GetTotalPay" datasource="#pds#">
					SELECT Sum(Credit) as TotalPay 
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND datetime1 <= #MyDate# 
				</cfquery>
				<cfquery name="GetCurBal" datasource="#pds#">
					SELECT Sum(Debit-Credit) as CurBal
					FROM transactions B 
					WHERE b.accountid = #accountid# 
					AND datetime1 <= #MyDate# 
				</cfquery>
				<cfquery name="GetLastAmount" datasource="#pds#">
					SELECT Credit FROM Transactions B 
					WHERE b.accountid = #accountid# 
					AND DateTime1 = #GetLastPay.MaxPayDate#
				</cfquery>
				<cfquery name="UpdCurChr" datasource="#pds#">
					UPDATE AgingTemp SET 
					CurChr = #GetCurChr.CurChr#, 
					Past30 = #GetPast30.Past30#, 
					Past60 = #GetPast60.Past60#, 
					Past90 = #GetPast90.Past90#, 
					LastPayDt = #GetLastPay.MaxPayDate#, 
					LastPayAm = #GetLastAmount.Credit#, 
					TotalPay = #GetTotalPay.TotalPay#, 
					CurBal = #GetCurBal.CurBal# 
