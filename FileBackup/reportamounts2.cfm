...
<cfquery name="CheckFor" datasource="#pds#" maxrows="100" debug="Yes">
	...
	FROM TransActions
	GROUP BY AccountID
	HAVING Sum(DebitLeft) > 0 
	AND Sum(CreditLeft) > 0
	ORDER BY AccountID 
</cfquery>
<cfif IsDefined("UpdateTheAmounts")>
	...
	<cfquery name="CheckFor2" datasource="#pds#">
		SELECT AccountID, SUM(Debit-Credit) as Bal1, SUM(DebitLeft-CreditLeft) as Bal2
		FROM TransActions 
		WHERE AccountID IN 
...
<cfif CheckFor.RecordCount GT 0 AND UpdateAmounts Is "0">
	<cfoutput>
		<META HTTP-EQUIV=REFRESH CONTENT="15; URL=reportamounts2.cfm?RequestTimeout=500<cfif UserID Is Not "">&UserID=#UserID#</cfif><cfif IsDefined("UpdateAmounts")>&UpdateAmounts=0</cfif>">
	</cfoutput>
</cfif>
...
<table border="#tblwidth#">
	...
	<tr>
		<td bgcolor="#tbclr#" colspan="3">This report will only show 100 max at a time.</td>
	</tr>
</table>
</cfoutput>
</center>
