...
<cfif IsDefined("UpdateIPAddress.x")>
	<cfloop index="B5" from="1" to="#LoopCount#">
		<cfset var1 = Evaluate("VarName#B5#")>
		<cfset var2 = Evaluate("SetupID#B5#")>
...
<cfif (IsDefined("DelIPs.x")) AND (IsDefined("DelThese"))>
	<cfquery name="DelData" datasource="#pds#">
		DELETE FROM Setup 
		WHERE SetupID In (#DelThese#) 
	</cfquery>
</cfif>
<cfif IsDefined("EnterIP.x")>
	<cfquery name="CheckFirst" datasource="#pds#">
		SELECT Value1 
		FROM Setup 
		WHERE VarName = 'AllowedNames' 
		AND Value1 = '#NewValue1#' 
	</cfquery>
	<cfif CheckFirst.RecordCount Is 0>
		<cfquery name="AddNewIP" datasource="#pds#">
			INSERT INTO Setup 
			(VarName, Value1, AutoLoadYN) 
			VALUES 
			('AllowedNames', '#NewValue1#', 0) 
		</cfquery>
	</cfif>
</cfif>
<cfif (IsDefined("DelTypes.x")) AND (IsDefined("DelThese"))>
	...
	<cfquery name="ResortData" datasource="#pds#">
		SELECT CardTypeID 
		FROM CreditCardTypes 
		WHERE ActiveYN = 1 
		ORDER BY SortOrder 
	</cfquery>
	<cfloop query="ResortData">
		<cfquery name="UpdData" datasource="#pds#">
...
<cfif IsDefined("EnterCardType.x")>
	<cfquery name="HowManyNow" datasource="#pds#">
		SELECT Max(SortOrder) As MSO 
		FROM CreditCardTypes 
	</cfquery>
	<cfset NextLine = HowManyNow.MSO>
	<cfif NextLine Is "">
		<cfset NextLine = 1>
	<cfelse>
		<cfset NextLine = NextLine + 1>
	</cfif>
	<cfquery name="AddData" datasource="#pds#">
		...
		VALUES 
		('#CardType#',0,0,#UseAW#,#UseOS#,1,#NextLine#,0)
	</cfquery>
	<cfquery name="ResetTheSort" datasource="#pds#">
		SELECT CardTypeID, SortOrder 
		FROM CreditCardTypes 
		WHERE ActiveYN = 1 
		ORDER BY SortOrder
	</cfquery>
	<cfloop query="ResetTheSort">
		<cfquery name="UpdData" datasource="#pds#">
			UPDATE CreditCardTypes SET 
			SortOrder = #CurrentRow# 
			WHERE CardTypeID = #CardTypeID# 
...
<cfif IsDefined("UpdateCreditCards.x")>
	...
	<cfloop index="B5" from="1" to="#LoopCount#">
		...
		<cfif IsDefined("SortOrder#B5#")>
			<cfset var6 = Evaluate("SortOrder#B5#")>
		<cfelse>
			<cfset var6 = 0>
		</cfif>
		<cfif var4 Is 0>
	...
	<cfquery name="ResortTypes" datasource="#pds#">
		SELECT CardTypeID 
		FROM CreditCardTypes 
		WHERE ActiveYN = 1 
		ORDER BY SortOrder 
	</cfquery>
	<cfloop query="ResortTypes">
		<cfquery name="UpdData" datasource="#pds#">
			UPDATE CreditCardTypes SET 
			SortOrder = #CurrentRow# 
			WHERE CardTypeID = #CardTypeID# 
...
<cfif IsDefined("UpdateTab1.x")>
   ...
   <cfelse>
	...
	<cfif Form.NoBOBHist Is 0>
		...
		<cfif CheckFirst.RecordCount Is 0>
			<cfquery name="AddNoHist" datasource="#pds#">
				INSERT INTO Setup 
				(VarName, Value1, Description, DateValue1, AutoLoadYN) 
				VALUES
				('NoBOBHist','1','Turn Off event logging.',Null,1) 
			</cfquery>
		<cfelse>
	...
	<cfelse>
		...
		<cfif CheckFirst.RecordCount Is 0>
			<cfquery name="TurnOnLoggin" datasource="#pds#">
				INSERT INTO Setup 
				(VarName, Value1, Description, DateValue1, AutoLoadYN) 
				VALUES
				('NoBOBHist','0','Turn Off event logging.',Null,0) 
			</cfquery>
		<cfelse>
			<cfquery name="TurnOnLogging" datasource="#pds#">
				UPDATE Setup SET 
				AutoLoadYN = 0, 
				Value1 = 0 
				WHERE VarName = 'NoBOBHist' 
	...
	<cfif Form.NoBOBPHist Is 0>
		...
		<cfif CheckFirst.RecordCount Is 0>
			<cfquery name="AddNoHist" datasource="#pds#">
				...
				(VarName, Value1, Description, DateValue1, AutoLoadYN) 
				VALUES
				('NoBOBPHist','1','Turn Off page logging.',Null,1) 
			</cfquery>
		<cfelse>
			<cfquery name="UpdHist" datasource="#pds#">
				UPDATE Setup SET 
	...
	<cfelse>
		...
		<cfif CheckFirst.RecordCount Is 0>
			<cfquery name="TurnOnLoggin" datasource="#pds#">
				...
				(VarName, Value1, Description, DateValue1, AutoLoadYN) 
				VALUES
				('NoBOBPHist','0','Turn Off page logging.',Null,0) 
			</cfquery>
		<cfelse>
			<cfquery name="TurnOnLogging" datasource="#pds#">
				UPDATE Setup SET 
