				...
				<table border="1">
					<tr bgcolor="#ttNTab#">	
						<form method="post" action="editcard.cfm">
							<input type="hidden" name="accountid" value="#AccountID#">
							<td <cfif tab Is "1">bgcolor="#ttSTab#"</cfif> ><input type="radio" name="tab" <cfif tab Is "1">checked</cfif> value="1" onclick="submit()" id="tab1"><label for="tab1">Misc</label></td>
							<cfif (TabPayByCk GT 0) OR (GetOpts.SUserYN Is 1) Or (ListFind(PayBys,'ck'))>
								<td <cfif tab Is "2">bgcolor="#ttSTab#"</cfif> ><input type="radio" name="tab" <cfif tab Is "2">checked</cfif> value="2" onclick="submit()" id="tab2"><label for="tab2">Check</label></td>
							</cfif>
							<cfif (TabPayByCd GT 0) OR (GetOpts.SUserYN Is 1) Or (ListFind(PayBys,'cd'))>
								<td <cfif tab Is "3">bgcolor="#ttSTab#"</cfif> ><input type="radio" name="tab" <cfif tab Is "3">checked</cfif> value="3" onclick="submit()" id="tab3"><label for="tab3">Check Debit</label></td>
							</cfif>
							<cfif (TabPayByCc GT 0) OR (GetOpts.SUserYN Is 1) Or (ListFind(PayBys,'cc'))>
								<td <cfif tab Is "4">bgcolor="#ttSTab#"</cfif> ><input type="radio" name="tab" <cfif tab Is "4">checked</cfif> value="4" onclick="submit()" id="tab4"><label for="tab4">Credit Card</label></td>
							</cfif>
							<cfif (TabPayByPo GT 0) OR (GetOpts.SUserYN Is 1) Or (ListFind(PayBys,'po'))>
								<td <cfif tab Is "5">bgcolor="#ttSTab#"</cfif> ><input type="radio" name="tab" <cfif tab Is "5">checked</cfif> value="5" onclick="submit()" id="tab5"><label for="tab5">Purchase Order</label></td>
							</cfif>
						</form>
					</tr>
				</table>
