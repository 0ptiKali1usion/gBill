...
<!--- This is the page that sets the variables for the scripts. 
		...
		Optional Parameters
			...
			LocSendCAuthID
--->
<!--- 4.0.1 01/04/01 The FTP ScriptCodes were able to be output as blank.  Put a catch in place to change blanks to the default.
		4.0.0 06/21/99 --->
<!--- RunVarValues.cfm --->
...
<cfquery name="LocFTP" datasource="#pds#">
	...
	FTPExecFile as perF22, FTPListDirs as perF23, FTPInheritD as perF24 
	FROM AccountsFTP 
	WHERE FTPID In 
		<cfif LocFTPID GT 0>
			(#LocFTPID#)
		<cfelseif IsDefined("LocAccntPlanID")>
			(SELECT FTPID 
			 FROM AccountsFTP 
			 WHERE AccntPlanID = #LocAccntPlanID#)
		<cfelse>
